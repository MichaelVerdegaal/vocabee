...
def search_vocabulary(search_query, vocabulary_collection):
    ...
    for row in vocabulary_collection['entries']:
        for col in ['kanji', 'kana', 'english']:
            ratio = fuzz.partial_ratio(search_query, row[col])
    ...
    perfect_matches = sorted(perfect_matches, key=lambda k: k['fuzzy_ratio'], reverse=True)
    matches = sorted(matches, key=lambda k: k['fuzzy_ratio'], reverse=True)
